...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: "Failed to refresh token", details: errorData }, { status: 400 });
    }

    const data = await response.json();

    // ✅ Set new access token in an HTTP-only cookie
    const responseWithCookie = NextResponse.json({ accessToken: data.access_token }, { status: 200 });
